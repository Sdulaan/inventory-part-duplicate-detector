...
test('E1 zero reviews disables meaningful reviewed output with truthful guidance', () => {
  ...
  assert.match(reviewedExportGuidance(result), /No human-confirmed same-identity sets are available yet/)
})

test('E2 only Reject decisions create no reviewed same-identity set', () => {
  const result = summarizeReviewedExportAvailability([state('KEEP_ALL_SEPARATE')])
  assert.equal(result.has_confirmed_sets, false)
  assert.match(reviewedExportGuidance(result), /Reject or Defer decisions create no reviewed identity set/)
})

test('E3 only Defer decisions create no reviewed same-identity set', () => {
...
test('E10 unreviewed system groups remain suggestions and are excluded from reviewed authority', () => {
  ...
  assert.match(panel, /system-suggested same-identity candidates for analysis and review/)
})
